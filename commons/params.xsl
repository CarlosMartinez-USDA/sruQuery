...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="vendorName" select="document($filePath)/*/mods:publisher[1]"/>
    <xsl:param name="archiveFile" select="tokenize($filePath, '/')[last()]" />
 ...
 <!--   <xd:doc scope="stylesheet">
    ...
    <xsl:param name="originalFilename" select="saxon:serialize(mods, $originalFilename)"/>
      <xsl:param name="workingDir" select="string('${pd}/${id}/${cfne}/')"/>
      <xsl:param name="workingDir" select="resolve-uri(unparsed-text('file:/C:/app/Github/PubData-XSLT/'))"/>
      <xsl:param name="workingDir"  select="saxon:sort(uri)"/>    
    <xsl:param name="originalFilename" select="saxon:system-id()"/>
    <xsl:param name="originalFilename" select="saxon:serialize(mods archiveFile)"/> 
 -->   
</xsl:stylesheet>